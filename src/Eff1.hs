...
t4 :: (Member (Reader Int) r, Member (Reader Float) r) => Eff r Float
t4 =
  liftM2
    (+)
    (local (+ (10 :: Int)) t2)
...
t_alttry =
  ([Right 10, Right 10, Right 10, Left "bummer1bummer2"] ==) $
    [ run . runError $
        (return 1 `add` throwError "bummer1")
          `alttry` return 10,
      run . runError $
        return 10
          `alttry` (return 1 `add` throwError "bummer2"),
      run . runError $
...
testCA :: (MonadPlus m, Integral b) => m b
testCA = do
...
test_ifte :: (Member NdetEff r, Integral b) => Eff r b
test_ifte = do
  n <- gen
  ifte
    ( do
...
tsplit :: (Member (Writer [Char]) r, Member NdetEff r, Num a) => Eff r a
tsplit =
  (tell "begin" >> return 1)
    `mplus` (tell "end" >> return 2)
...
ter1 :: Bool
ter1 =
  ((Left "exc" :: Either String Int, 2) ==) $
    run $ runState (runError tes1) (1 :: Int)

ter2 :: Bool
ter2 =
  ((Left "exc" :: Either String (Int, Int)) ==) $
...
ter3 :: Bool
ter3 =
  ((Right "exc" :: Either String String, 2) ==) $
    run $ runState (runError (teCatch tes1)) (1 :: Int)

ter4 :: Bool
ter4 =
  ((Right ("exc", 2) :: Either String (String, Int)) ==) $
...
-- The signature is inferred
mapMdebug' ::
  (Show a, MemberU2 Lift (Lift IO) r) =>
  (a -> Eff r b) ->
  [a] ->
  Eff r [b]
mapMdebug' _ [] = return []
